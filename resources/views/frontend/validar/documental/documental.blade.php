	...
	<div class="col-12">
		<div class="card">
            ...
            <div class="card-header bg-info">
                <h5 class="text-white float-left">Evaluación Documental</h5>
                <a href="{{route('concursos')}}" class="btn btn-outline-light btn-sm float-right"><i class="fas fa-plus"></i> Concursos</a>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <form action="{{ route('documental.search') }}" class="form-horizontal" method="POST">@csrf
                            @include('frontend.validar.documental.form-documental')
                ...
                <div class="row border-top">
                    <div class="col-sm-12 card-header bg-info">
                        @if ($datos->concurso !== null)
                            <h5 class="text-white float-left">Concurso Nº {{ $datos->concurso->id }} - {{ $datos->concurso->decripcion }} - {{ $datos->concurso->cargos->descripcion }}</h5>
                            @if ($datos->fechas !== null)
                            <span class="text-white">Fechas: <b>{{ $datos->fechas }}</b></span>
                            @endif
                            <span class="text-white float-right">Estado: <b>{{ $datos->estado['dscrp'] }}</b></span>
                        @endif
                    </div>
                </div>
                <br />
                    ...
                    <table class="table table-striped table-bordered" id="data-table">
                        ...
                        <tbody>
                            @if($datos->postulacion !== null)
                                @include('frontend.validar.documental.data-table')
                            @endif
                        </tbody>
                        <tfoot>
                            <tr>
